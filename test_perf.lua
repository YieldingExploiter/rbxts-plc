...
local function pst(st) for i = 1, 8 do pf('x[%d]:  %08X     c[%d]:  %08X', i, st.x[i], i, st.c[i]) end end
...
local function perf_xtea()
  ...
  for i = 1, #plain // 8 do xtea.encrypt_u64(st, 0xaaaa5555aaaa5555) end
  done()
  --
end -- perf_xtea
...
local function perf_norx()
  ...
  for j = 1, 1 do
    ...
    for i = 1, cnt do ct[i] = norx.aead_encrypt(k, n, pt[i]) end
    done()
    start('norx decrypt', '10mb, 4k messages')
    for i = 1, cnt do local p = norx.aead_decrypt(k, n, ct[i]) end
    done()
  end
  --
end -- perf_norx
